...
type Columns = ResizableColumnsType<ProColumns<GithubIssueItem>[]>
...
export const Component = () => {
  ...
  const { components, resizableColumns, tableWidth, refresh } = useAntdResizableHeader({
    columns,
  })

  const [columnsState, setColumnsState] = useState<Record<string, ColumnsState>>()
  const previousColumnsState = usePrevious(columnsState)

  useUpdateEffect(() => {
    if (!isEqual(columnsState, previousColumnsState)) {
      refresh()
    }
  }, [columnsState])
